...
const ChecklistEntradaPage = () => {
  ...
  const carregarChecklist = useCallback(async () => {
    setLoading(true);
    try {
      const res = await checklistService.get(turnoId, 'entrada');
      ...
      console.error('Erro ao carregar checklist de entrada:', err);
    } finally {
      setLoading(false);
    }
  }, [turnoId]);
  ...
  const abrirPergunta = async (pergunta) => {
    try {
      ...
      console.error('Falha ao travar pergunta (entrada):', err);
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-3">
              ...
              {itens.length === 0 && (
                <div className="text-sm text-gray-500">Nenhum item de check-in disponível para seus setores.</div>
              )}
            </div>
            ...
            <div className="p-4 border rounded-lg">
              <div className="flex items-center justify-between">
                <div>
                  <div className="font-medium">Contagem do Inventário (Entrada)</div>
